...
import { viewContactListServices } from "../../services/viewContactList";

const useViewContactListHelper = () => {
    ...
    const resources = useMemo(() => {
        return {
            title: t("viewContact.title"),
            cardInfo: {
                title: t("viewContact.cardInfo.title"),
                description: t("viewContact.cardInfo.description"),
            },
            textButton: t("viewContact.button"),
            textButtonTwo: t("viewContact.buttonCancel"),
            table: {
                campaigns: t("viewContact.table.item[0]"),
                channel: t("viewContact.table.item[1]"),
                dateStart: t("viewContact.table.item[2]"),
                dateEnd: t("viewContact.table.item[3]"),
                week: t("viewContact.table.item[4]"),
                hoursStart: t("viewContact.table.item[5]"),
                hoursEnd: t("viewContact.table.item[6]"),
                amount: t("viewContact.table.item[7]"),
                state: t("viewContact.table.item[8]"),
                action: t("viewContact.table.item[9]")
            },
            titleButtonPrimary: t("viewContact.buttonSave"),
            titleButtonSecundary: t("viewContact.buttonCancel")
        };
    }, [t]);
    ...
    const goDeleteContact = (value: any) => {
        setParams(value)
    }
    ...
    const { isLoading, isRefetching, data, refetch, remove } = useQuery(
        ...
        () => {
            if ((companyId !== undefined || companyId !== "") && auth?.urlEnviroments) {
                return viewContactListServices.postPagedGeral(cookieValue, listParamsEmail, auth?.urlEnviroments.HOST.API_DIGITAL_URL);
            }
        },
        {
    ...
    const { } = useQuery(
        ...
        () => {
            if ((companyId !== undefined || companyId !== "") && auth?.urlEnviroments) {
                return viewContactListServices.getState(cookieValue, companyId, auth?.urlEnviroments.HOST.API_DIGITAL_URL);
            }
        },
        {
                ...
                return (
                    <Box sx={{ display: "inline-flex", gap: "10px" }}>
                        <Fab size="small" sx={{ mb: 1, mr: 1 }} color="primary" aria-label="add">
                            <Tooltip title="Apagar Campanha" onClick={() => goDeleteContact(params)}>
                                <IconButton>
                        ...
                        <Fab size="small" sx={{ mb: 1, mr: 1 }} color="secondary" aria-label="add">
                            <Tooltip title="Visualizar o HTML do Contatos">
                                <IconButton>
                                    <HtmlIcon sx={{ color: "#fff" }} />
                                </IconButton>
                            </Tooltip>
                        </Fab>
                        <Fab size="small" sx={{ mb: 1, mr: 1 }} color="warning" aria-label="add">
                            <Tooltip title="Iniciar processo">
                                <IconButton>
...
export default useViewContactListHelper;
