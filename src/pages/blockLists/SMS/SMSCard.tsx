...
import SMSCardCustomToolbar from "./shards/SMSCardCustomToolbar"
import useSMSBlackListManagementHelper from "./SMSBlackList.helper"
...
const SMSCard = () => {
    const {
        ...
        validatePTPhone, 
        resources
    } = useSMSBlackListManagementHelper();
    ...
    const handleInsertSMS = () => {
        setPhoneValidated(validatePTPhone(phoneRef?.current?.value))     
        
        if(validatePTPhone(phoneRef?.current?.value)) {
            const smsBlock: any = {
                companyId: compId, //auth?.companyId !== undefined ? String(auth?.companyId) : "";,
                mobileNumberToAdd: phoneRef?.current?.value || '',
                createdBy: auth?.infoUser?.email || ""
            }
    
            try {
                if (token) {
                    const result = postHttp(api, '/AddSMSBlock', smsBlock)
                    return result;
                }
                else {
                    return null;
                ...
                >              
                    ...
                    <Box sx={{display: 'flex', alignItems: 'center', padding: { lg: '1rem 1rem', md: '1rem 0.5rem', sm: '0.5rem 0.5rem 0 0.5rem', xs:'0.5rem 0.5rem 0 0.5rem' }  }}>
                        ...
                        <Box sx={{height: '30px', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                            <CustomInput type='number' inputRef={phoneRef} placeholder='000 000 000' sx={{margin: '0 1rem'}} inputMode="tel"/>
                            {!phonevalidated && <Box sx={{color: 'red', fontSize:'small'}}>{resources.inputError}</Box>}
                        </Box>
                        <Button 
                            sx={{
                                height: '2rem',
