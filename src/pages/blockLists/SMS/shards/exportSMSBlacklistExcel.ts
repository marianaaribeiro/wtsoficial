...
export const handleDownloadExcel = (
    ...
    xlsx.writeFile(workBook, "SMSBlackList.xlsx");
...
export const getSMSBlacklistDataTableExcel = async (params: any, cookieValue: any, auth: any, setValueErrorFilter: (e: boolean) => void,
    ...
    try {
        if (auth?.urlEnviroments) {
            ...
            setTimeout(() => {
                handleDownloadExcel(response.blackListedSMS || [], setValueErrorFilter,
                    setloadingExcel)
            }, 300);
        }
    } catch (error) {
