...
const handleDownloadCSV = async (valuesCSV: any[],
    ...
    donwloadBlob(csv, 'text/csv;charset=utf-8;', 'SMSBlackList')
...
export const getSMSBlacklistDataTableCSV = async (params: any, cookieValue: any, auth: any, setValueErrorFilter: (e: boolean) => void,
    setloadingCSV: (e: boolean) => void, setOpenAlertCSV: (e: any) => void) => {
  
    try {
        if (auth?.urlEnviroments) {
            const response = await smsBlackListManagementServices.getSMSBlackListPaged(cookieValue, params, () => {}, auth?.urlEnviroments.HOST.API_DIGITAL_URL);

            setTimeout(() => {
                handleDownloadCSV(response.blackListedSMS || [], setValueErrorFilter,
                    setloadingCSV)
            }, 300);
        }
    } catch (error) {
