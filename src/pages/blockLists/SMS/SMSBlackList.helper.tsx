...
import { IBlacklistSMS, IBlackListSMSReponse, IColumnInformation } from "./IProps";
import { filterData } from "./shards/filterData";
import { smsBlackListManagementServices } from "../../../services/smsBlackList";

const useSMSBlackListManagementHelper = () => {
    ...
    const [rowTable, setRowTable] = useState<IBlacklistSMS[]>([])
    ...
    const [listParamsSMS, setListParamsSMS] = useState({
    ...
    const resources = useMemo(() => {
        return {
            ...
            inputTitle: t("blacklists.sms.inputTitle"),
            inputError: t("blacklists.sms.inputError"),
            buttonInsert: t("blacklists.sms.buttonInsert"),
            searchButton: t("blacklists.sms.searchButton"),
            cleanFilter: t("blacklists.sms.cleanFilter"),
            tableHeader: {
                Id: t("blacklists.sms.tableHeader.Id"),
                CompanyId: t("blacklists.sms.tableHeader.CompanyId"),
                Msisdn: t("blacklists.sms.tableHeader.Msisdn"),
                CreatedOn: t("blacklists.sms.tableHeader.CreatedOn"),
                CreatedBy: t("blacklists.sms.tableHeader.CreatedBy"),
                Actions: t("blacklists.sms.tableHeader.Actions"),
            }
        };
    }, [t]);
    ...
    function validatePTPhone(phoneToValidate: string) {
        const re = /^(?:2\d|9[1236])[0-9]{7}$/;
        return re.test(phoneToValidate);
    }

    /* convert value table N/A */
    const convertRowData = (valuesData: IBlacklistSMS[] & any) => {
        const valueTable = valuesData && valuesData?.map((itemData: IBlacklistSMS) => {
            return {
                id: `${itemData?.id ? itemData?.id : ""}`,
                Id: `${itemData?.id ? itemData?.id : ""}`,
                CompanyId: `${itemData?.companyId ? itemData?.companyId : ""}`,
                Msisdn: `${itemData?.msisdn ? itemData?.msisdn : ""}`,
    ...
    const { isLoading, isRefetching, data, refetch, remove } = useQuery(
        ...
        () => {
            if (companyId !== undefined || companyId !== "") {
                return smsBlackListManagementServices.getSMSBlackListPaged(cookieValue, listParamsSMS, informationError, envUrl);
            }
        },
        {
            enabled: true,
            ...
            retry: false,
            retryDelay: 1,
            onSuccess: (data: IBlackListSMSReponse) => {
                if (data) {
                    setRowTable(convertRowData(data.blackListedSMS || []));
    ...
    const handleDelete = () => {
        try {
            if (token) {
                const result = postHttp(api, '/DeleteSMSBlock', {
                    companyId: compId, //companyId
...
export default useSMSBlackListManagementHelper;
